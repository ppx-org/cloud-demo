...
public class ExcelTitle implements Serializable {
    ...
    */
	...
	public String getWidth() {
		if(width==null)
			if(this.code != null && "IDCARD".equalsIgnoreCase(this.code)){
				width=String.valueOf(this.getName().length()*8);
			}else{
				width=String.valueOf(this.getName().length()*6);
			}
    ...
    public List<Map> getChangerList() {
    	if(changerList==null&&!StringUtils.isEmpty(changer)){
    		//默认从数据字典中获取转换类
    		//package com.ppx.cloud.demo.module.test.excel;changerList=DBUtils.getBaseCodeWithNouse(this.changer);
    	}
		return changerList;
	}
